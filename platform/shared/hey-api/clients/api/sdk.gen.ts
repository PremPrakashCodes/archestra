...
/**
 * Get a publicly-shared conversation by its share token
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * None (no additional RBAC permission required)
 */
export const getPublicSharedConversation = <ThrowOnError extends boolean = false>(options: Options<GetPublicSharedConversationData, ThrowOnError>) => (options.client ?? client).get<GetPublicSharedConversationResponses, GetPublicSharedConversationErrors, ThrowOnError>({ url: '/api/chat/public/share/{token}', ...options });
